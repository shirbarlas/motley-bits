...
class SplitReviews(object):
    ...
    def _init_who_can_review_dict(self):
        self.who_can_review_each_commit = {committer: self.who_can_review_committer(committer) for committer in self.committers}

    def who_can_review_committer(self, committer):
        possible_reviewers = []
        for reviewer in self.reviewers.keys():
            if reviewer.group != committer.group and committer not in self.reviewers[reviewer]:
    ...
    def choose_reviewer(self, committer):
        ...
        for reviewer in self.who_can_review_each_commit[committer]:
            ...
            elif len(self.reviewers[reviewer]) < len(self.reviewers[chosen_reviewer]):
                chosen_reviewer = reviewer
    ...
    def remove_possible_reviewer(self, committer, reviewer):
        if reviewer is None:
            print "ERROR: could not find any more legal reviewers for %s" % committer
        else:
...
def main(argv):
    ...
    try:
        ...
        opts, args = getopt.getopt(argv, "hc:r:n:", ["committers=", "reviewers="])
